    ...
    <div class="row">
        <div class="col-12 col-lg-8">
            <div class="course-content-body user-select-none">
                ...
                @include('web.default.includes.discounts.instructor_discounts_card', ['discount' => $instructorDiscount,
                ...
                <div class="mt-50">
                    <ul class="nav nav-tabs bg-light-gray rounded-sm p-15 d-flex align-items-center justify-content-between"
                        ...
                        <li class="nav-item">
                            <a class="position-relative font-14 text-dark {{ (request()->get('tab','') == 'content') ? 'active' : '' }}"
                        ...
                        <li class="nav-item">
                            <a class="position-relative font-14 text-dark {{ (request()->get('tab','') == 'reviews') ? 'active' : '' }}"
                                id="reviews-tab" data-toggle="tab" href="#reviews" role="tab" aria-controls="reviews"
                                aria-selected="false">{{ trans('product.reviews') }}
        ...
        <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
            <div class="rounded-lg shadow-sm">
                <div class="course-img {{ $course->video_demo ? 'has-video' :'' }}">
                    ...
                    <img src="{{ $course->getImage() }}" class="img-cover bg-white" alt="">
                ...
                <div class="px-20 pb-30">
                    <form action="/cart/store" method="post">
                        ...
                        <div class="mt-20 d-flex flex-column">
                            ...
                            <button type="button" data-slug="{{ $course->slug }}"
                                class="btn new-btn rounded-pill {{ (!$authUserJoinedWaitlist) ? ((!empty($authUser)) ? 'js-join-waitlist-user' : 'js-join-waitlist-guest') : 'disabled' }}"
                            ...
                            <a href="{{ $course->getLearningPageUrl() }}"
                                class="btn new-btn rounded-pill">{{ trans('update.go_to_learning_page') }}</a>
                            @elseif(!empty($course->price) and $course->price > 0)
                            <button type="button"
                                class="btn new-btn rounded-pill {{ $canSale ? 'js-course-add-to-cart-btn' : ($course->cantSaleStatus($hasBought) .' disabled ') }}">
                                @if(!$canSale)
                            ...
                            @if(!empty($installments) and count($installments) and
                            getInstallmentsSettings('display_installment_button'))
                    ...
                    <div class="mt-35">
                        <strong
                            class="d-block text-orange font-weight-bold">{{ trans('webinars.this_webinar_includes',['classes' => trans('webinars.'.$course->type)]) }}</strong>
            ...
            {{-- Gift Card --}}
            @if($course->canSale() and !empty(getGiftsGeneralSettings('status')) and
...
@if(!empty($course->creator) and !empty($course->creator->getLiveChatJsCode()) and
